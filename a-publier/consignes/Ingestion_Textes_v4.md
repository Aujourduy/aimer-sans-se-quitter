# Carte d'ingestion des textes — pour Claude Code (v10)
*Duy DANG. Comment (re)remplir les corps de texte du site à partir des corpus sources, conformément à `Selection_Textes_Site_v10.md`.*

S'utilise avec : `Selection_Textes_Site_v10.md` (la carte des textes, catégories, niveaux, marqueurs) et les fichiers sources ci-dessous. Le site (structure Astro, pages de thème, navigation, design) existe déjà ; cette passe **régénère uniquement le contenu** `src/content/textes/`.
...
Supprimer tous les fichiers existants de `src/content/textes/` puis régénérer depuis `Selection_Textes_Site_v10.md`. Ainsi les textes que la v10 archive ou écarte disparaissent d'eux-mêmes.
...
Créer un fichier **uniquement** pour les textes listés dans les sections de revue de la v10 :
...
**Ne créer aucun fichier** pour la section **« Écartés »** de la v10. (Il n'y a plus de section « Archive intime » : les textes intimes sont revenus dans la catégorie *Désir et intimité*, certains en `draft`.)
...
- **`category`** : la section v10 → `amour-presence` · `desir-verite` · `peur-masque` · `fables-paradoxes` · `desir-intimite`.
- **`order`** : numérotation continue dans la catégorie, dans l'ordre d'apparition v10 (En tête, puis Au cœur, puis En appui). Premier = 1.
- **`entry` / `entryRole`** : `true` + rôle (`reconnaissance` / `deplacement` / `mecanisme`) pour les trois entrées seulement.
- **`excerpt`** : la glose après le tiret « — » de la ligne v10 (reformulée en une phrase sépia ≤ 16 mots si besoin). Ne pas inventer au-delà.
- **`draft`** (règle unique) : `draft: true` si la ligne v10 porte `[brouillon]`, `[édit]`, `[édit léger]`, `[édit fort]`, `[sensible]`, `[biographique léger]`, `[accord cliente]`, **ou** s'il est annoté « à relire / titre revu / titre à revoir / (aphorisme) ». Sinon `draft: false`. Le marqueur **`[intime]` est informatif et ne déclenche PAS le brouillon** (ces textes sont publiés). *Désir et intimité* n'est plus une catégorie automatiquement en brouillon : suivre le marqueur de chaque ligne.
- **`draftReason`** (seulement si `draft: true`) : raison courte, déduite du marqueur v10 —
  ...
  - `[brouillon]` (en *Désir et intimité*) → « texte intime explicite, non surfacé pour l'instant »
  - `[brouillon, fin à compléter]` → « fin à compléter »
  - `[intime]` → pas de brouillon, pas de `draftReason` (publié)
...
**Renommages et cas particuliers de la v10 :**
...
**Doublons — ne créer qu'un fichier (déjà géré par la v10, mais vigilance) :**
...
## 5. Rappels transverses (de la v10)
